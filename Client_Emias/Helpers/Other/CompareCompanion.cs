...
namespace Client_Emias.Helpers.Other
{
    static class CompareCompanion
    {
        ...
        public static bool IsDateEarlier(DateTime date, DateTime sourceDate)
        {
            return sourceDate.Year > date.Year ||
                sourceDate.Year == date.Year && sourceDate.Month > date.Month ||
                sourceDate.Year == date.Year && sourceDate.Month == date.Month && sourceDate.Day > date.Day ||
                sourceDate.Year == date.Year && sourceDate.Month == date.Month && sourceDate.Day == date.Day && sourceDate.TimeOfDay.Ticks > date.TimeOfDay.Ticks;
        }
        public static bool IsDateEarlier(DateOnly date, DateOnly sourceDate)
        {
            return sourceDate.Year > date.Year ||
                sourceDate.Year == date.Year && sourceDate.Month > date.Month ||
                sourceDate.Year == date.Year && sourceDate.Month == date.Month && sourceDate.Day > date.Day;
